...
string costUsageResourceStackName;
string globalServiceRolesPermissionsStackName;
...
string costUsageResourceTemplatePath = "Cost_Usage_Report_CFN_Template.json";
...
Task("Initialise")
    .Does(() => {
        ...
        costUsageResourceStackName = $"rx-gbs-datalake-cost-usage-{environment}";
...
//        foreach(var file in files){
//           scriptsToDeploy.Add(file.FullPath,"governance/scripts/"+file.GetFilename());
//         }
//        await upload_to_code_bucket(awsCredentials, region, codeBucketName, scriptsToDeploy);
//    });


Task("costUsageResources")
    .IsDependentOn("Initialise")
    .Description("Cost&Usage Resource Setup")
    ...
    .Does(() => {
        ...
        var result = RunCloudFormation(
            new RunCloudFormationSettings
            {
                ...
                Region = region,
                StackName = costUsageResourceStackName,
                TemplatePath = costUsageResourceTemplatePath,
...
Task("Default")
    .IsDependentOn("costUsageResources");
